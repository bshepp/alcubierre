...
- [ ] **2D.5 Fell-Heisenberg horizon and CTC analysis** — for the canonical WEC+DEC-passing point $(V=1.5, \sigma=10, m_0=3, a=0.05, \ell=4, r=9)$, compute (i) $\partial_t$ timelike-norm (foliation health), (ii) outward null geodesic expansion (trapped surfaces), (iii) Everett-Roman 1997 §4 CTC test. The most likely place a "too good to be true" objection lands. Effort: 1-2 sessions.
- [ ] **2D.6 Independent FH pipeline re-implementation** — replicate the WEC+DEC-passing result in a second pipeline (different stencil-of-stencils, or Mathematica xAct) to rule out a finite-difference systematic. Highest-value cheap check before any external claim. Effort: 1-2 sessions.
- [ ] **2D.7 FH source-matter classification** — given $T_{\mu\nu}^{\rm FH}$, place the principal-pressure profile $(\rho, p_1, p_2, p_3)$ in the Bobrick-Martire 2021 §III taxonomy (perfect fluid? anisotropic? plasma? scalar condensate?). Effort: 1 session.
- [ ] **2D.8 FH asymptotic matching + double-bubble CTC test** — match box-edge interior to Schwarzschild/Minkowski exterior; check whether matching needs a negative-energy Israel junction. Independently: do two FH bubbles in opposite directions form CTCs? Effort: 1-2 sessions.
