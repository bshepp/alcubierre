...
- [ ] **2D.5 Connectivity and topology of the WEC+DEC-passing region** — analyse the 1404 strict-pass points (or, more powerfully, run a denser sweep at higher `Npts` over the band-centre $(\sigma, m_0, a, \ell, r) \in [4, 10] \times [2.5, 3.5] \times [0.05, 0.5] \times [2, 8] \times [4, 9]$) to determine: (i) Are the WEC+DEC-passing points a single connected region in $(\sigma, m_0, a, \ell, r)$-space, or several disconnected islands? Test by 2D pairwise projections and connected-component analysis on the discrete sweep grid. (ii) If connected, characterise the boundary surface — does the slack vanish smoothly, suggesting a closed-form sub-family lurks at the boundary? (iii) Symmetry analysis — does the passing region respect any obvious group action on the parameters (scaling, reflection)? **Why this is critical**: an analytic sub-family (one that can be derived in closed form rather than discovered by sweep) is **substantially more defensible in peer review than "we swept and found 1404 hits."** A connected region whose boundary has clean structure is the prerequisite for finding such a sub-family. Effort: 1 session, no new compute (re-analyses the existing parquet); +0.5 session if a denser refinement sweep is needed. Zero additional cost beyond the existing dataset.
