...
| Krasnikov-tube prior art makes "static infrastructure" claims unoriginal | **Realised, 2026-04-16 (Session 7)** | Low — speculation document only, not core program | Documented in `KRASNIKOV_TUBE_NOTES.md`. Krasnikov 1995 / Everett–Roman 1997 already established static-network warps with full classical stress-energy + CTC theorem. Mitigation completed by Task 2A.13 (Session 8): ported Everett–Roman's classical result into our framework with universal scaling law $\rho_p^{\min} = -0.122\,\eta/\epsilon^2$ and zero-WEC-pass-rate sweep. Speculation document closed rigorously. |
