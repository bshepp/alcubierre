...
- [ ] **2D.5e Hard Fix: symbolic extraction of the closed-form boundary equation** — **PROMOTED to active medium-priority by Session 14 §10 finding** that the polynomial-fit programme has reached its useful endpoint (degree-4 fit is dense, no sparse closed-form). Documented in [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §8. Symbolic Hessian + ADM stress-energy + interior $(X,Y,Z)$ minimisation gives the exact transcendental boundary equation $S(\sigma, m_0, a, \ell, r) = 0$ in closed form (containing exp + erf factors from the FH potential). Effort: **3-5 sessions** of heavy SymPy work; the interior minimisation is the hardest step.
