...
- [ ] **2D.5b Extract the cubic implicit boundary equation $P_3(\sigma, m_0, a, \ell, r) = 0$** — *opened by Session 12 §7.7 finding that the boundary IS approximately a low-degree polynomial surface*. Refit logistic at degree 3 with no regularisation, extract the 55 coefficients, drop terms at the noise level, check for clean dimensionless interpretation. Yields a closed-form analytic sub-family for $\partial\mathcal{M}$. Effort: 1 session, no new compute.
