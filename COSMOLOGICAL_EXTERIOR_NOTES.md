...
**Update — Session 46 (2026-07-06): the G-Z construction has been reproduced and certified** ([`GARATTINI_ZATRIMAYLOV2025_EVALUATION.md`](GARATTINI_ZATRIMAYLOV2025_EVALUATION.md); battery [`verification/test_gz_desitter_reproduction.py`](verification/test_gz_desitter_reproduction.py), 9/9). Confirmed exactly, with sharp caveats: the matched bubble is *exactly comoving* (zero transport beyond the Hubble flow), the "averaged" conditions are fixed-$t$ volume averages only, and ANEC along wall-crossing null geodesics is strictly violated. The 5b qualifier stands, A-grade within slice, but opens no useful-warp loophole.
