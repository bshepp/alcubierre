...
- **0 / 1456** augmented points improve `dec_slack_min`; **0 / 1456** improve `wec_slack_min`; **0** strict passes *(⚠ Session-38: this gate was vacuous — the baseline itself fails DEC at Npts=49, a resolution artifact; see §5)*; **0** points with `passenger_zone_radius > h`.
...
**Does not establish:** ~~behaviour off the canonical FH anchor~~ *(Session 38 closed this: four stratified genuinely-strict-pass anchors, §5)*; A-structures with *independently varied* asymmetry/exponent $(m_{0A}, a_A, \ell_A, \Pi_A)$; sums of several FH modes *within* one component; radii outside $\{6, 9\}$ or $\sigma_A$ outside $\{5, 10\}$. Re-open only if a downstream finding suggests one of these restrictions is uniquely binding.
...
**Task 2D.11 verdict (slice-scoped): CLOSED NEGATIVE.** Within the static smooth-N slice at the canonical anchor, spanning perturbative to non-perturbative amplitudes, vorticity never helps and generically harms; the passenger zone is `radius = h` in every one of the augmented evaluations across all three families. Phase 3 removes the escape hatch the first two negatives shared: Phases 1–2 could be read as "the envelope's support never overlapped the violating cells *helpfully*"; Phase 3's $\vec A$ has the same multi-scale FH wall structure as the bubble itself — maximal structural overlap — and the result is uniform strict degradation, not repair. Within this slice, the irrotational restriction $\vec N = \nabla\phi$ is **not** the load-bearing driver of the §9 "all wall, no interior" pathology; adding $\vec\nabla\times\vec A$ of any tested shape only spends energy-condition margin. The deferred full sweeps (joint FH+vortical variation, Phase-1/2/3 `*_full` configs) stay undispatched — three independent definitive-at-anchor negatives do not justify the spend; re-open per §1.3 / §2.3 / §3.3 criteria. **Session 38 re-based this verdict on informative evidence at four genuinely strict-pass anchors — see §5; the verdict stands.**

## §5 Session-38 multi-anchor kill-test (Session-35 audit queue item (c)) — verdict re-based and upheld

**Why.** The Session-35 audit flagged (W3) that the §3 closure augmented a single anchor whose baseline was itself DEC-violating at the run's Npts=49 — so gate (c) "0 strict passes" carried no information — and that the anchor's V=0.5 matched no certified sweep row.

**Resolution diagnosis first.** The kill-test resolved both provenance puzzles before running anything: (i) the canonical anchor's *structure* (σ=10, m₀=3, a=0.05, ℓ=4, r=9) is certified **strict-pass at Npts=65 for every V on the Session-11 grid** — the recorded "DEC-violating baseline" (dec_slack −7.74e-2) was an **Npts=49 resolution artifact**, not a bad anchor; (ii) the V=0.5 choice was immaterial because both slacks scale exactly as $V^2$ across the certified sweep (strict-pass signs are V-invariant — the recorded 1404 strict-pass rows split as exactly 234 at each of the six V values). Session 33's *differential* claims (augmented vs baseline on the same grid) were unaffected; its absolute numbers and the strict-pass gate carried the artifact.

**Design.** Re-ran the §3 multimode preview grid (identical vortical axes) at **Npts=65** — the certification resolution — at four stratified anchors that are actual rows of the Session-11 sweep (configs `fell_heisenberg_vortical_multimode_kill_{a1,a2,a3,a3s,b1}.json`): A1 deep-pass (V=1.5, σ=10, m₀=3, a=0.2236, ℓ=6, r=9; margins wec +0.0374/dec +0.0187), A2 mid (V=0.94, same structure), A3 thin-margin (V=0.10, same structure; dec margin **+8.3e-5**, the CTC-free-tail amplitude class), B1 structurally diverse (V=1.5, σ=6, r=7.75), plus A3s = A3 with V-scaled amplitudes ($|V_{A,i}| \le 0.04$). 7285 evaluations total.

**Gates (every anchor):** baseline row reproduces its certified sweep row (rel dev ≤ 4×10⁻⁵ for A1/A2/B1; 1.6×10⁻³ relative = ~10⁻⁷ absolute on A3's tiny margin); baseline genuinely strict-passes at the run's own Npts. Then, over the augmented points at each anchor:

| anchor | augmented pts | improve dec | improve wec | **retain strict-pass** | passenger_R > h | both slacks strictly worse |
|---|---|---|---|---|---|---|
| A1 (V=1.5, deep) | 1456 | 0 | 0 | **1456 (100%)** | 0 | 1456 (100%) |
| A2 (V=0.94) | 1456 | 0 | 0 | **1456 (100%)** | 0 | 1456 (100%) |
| A3 (V=0.10, margin 8.3e-5, $|V_{A}|$ up to 3× the FH amplitude) | 1456 | 0 | 0 | **1456 (100%)** | 0 | 1456 (100%) |
| B1 (V=1.5, σ=6, r=7.75) | 1456 | 0 | 0 | **1456 (100%)** | 0 | 1456 (100%) |
| A3s (V=0.10, scaled $|V_{A}| \le 0.04$) | 1456 | 0 | 0 | **1456 (100%)** | 0 | 1456 (100%) |

Worst-case dec-slack degradation per anchor: −3.5×10⁻⁵ (A1), −2.5×10⁻⁵ (A2), −1.1×10⁻⁵ (A3), −2.9×10⁻⁵ (B1) — approximately ∝ V·V_A (cross-term dominated); the pure-$V_A^2$ collapse Session 33 measured (dec → −0.408) is a *non-perturbative* ($|V_{A}|$ up to 3) phenomenon, negligible at $|V_{A}| \le 0.3$.

**What changes and what stands.** *Changes:* the §3 gate line "0 strict passes" is superseded — at genuinely strict-pass anchors, augmentation at the preview amplitudes retains strict-pass in 100% of 7280 points; the recorded zero was baseline-inherited. *Stands, now on informative evidence:* vorticity **never improves either slack** (0/7280 at four genuine anchors + 0/2912 at the original anchor), **universally degrades both** (100% everywhere), and **never extends the passenger zone** — `passenger_zone_radius = h` (= 0.375 at Npts=65) for all 7285 rows including baselines. Since 2D.11's target pathology was the zero-volume passenger zone, and that pathology is present at every certified strict-pass anchor with or without vorticity, **the CLOSED NEGATIVE verdict is upheld**: the irrotational restriction is not the driver of "all wall, no interior"; vorticity only spends EC margin.

**Files.** Configs `hf_jobs/configs/fell_heisenberg_vortical_multimode_kill_*.json`; artifact `sweeps/fell_heisenberg_vortical_multimode_kill_concat.parquet` (negation-tracked, 7285 rows); analysis in SESSION_LOG Session 38.
