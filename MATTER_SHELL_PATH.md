...
|---|---|---|---|---|---|
| 1 | **Alternate shift families** | Shift is Alcubierre $\beta^x \hat x$ | [`shift_families.ipynb`](shift_families.ipynb) | [`SHIFT_FAMILIES_NOTES.md`](SHIFT_FAMILIES_NOTES.md) | **DONE 2026-04-17 (Session 9); rebuilt analytic 2026-07-05 (Session 36** — the Session-9 evaluator had a frame-projection bug; all four families are now closed by identity for every parameter/profile, corrected sweeps 0/140 + 0/2496 concur; see the NOTES correction section**)**. Lentz 2020 is not contradicted because his construction is genuinely 3D multi-mode + plasma source, outside this slice. |
...
| 3 | **Time-dependent acceleration** | Steady-state metric + Lorentz boost is sufficient | [`time_dependent.ipynb`](time_dependent.ipynb) | [`TIME_DEPENDENT_NOTES.md`](TIME_DEPENDENT_NOTES.md) | **DONE 2026-04-17 (Session 9).** $\dot v$ correction to $\rho_p$ is antisymmetric in $x$, scales as $1/\tau$, with peak ratio to static $\rho_p$ of 0.003 at $\tau = R/c$. Net momentum injection at quadrupole order is zero by symmetry. Package 3 conclusions transfer. TRUST_AUDIT #5 closed Session 37 via `verification/test_sxs_kick_pull.py` (catalog-wide max NR kick 3119 km/s < the 5000 km/s input — ceiling conservative; the Colab cell's target simulation was mis-designated). |
...
| P2.5 | **(Resolved, 2026-04-16.)** How does the required shell mass scale with desired $\beta_\text{warp}$ and $R$? | Addressed in `thickness_bound.ipynb`. The scaling law is $\Delta_{\min}/R = \kappa\beta/C$ with analytic $\kappa \in [0.05, 0.875]$ (upper bound refined from 0.75 by 2A.9a, Session 15c; the full thick-shell numerical value is $\kappa \approx 4.2$–$5.8$ per 2A.9b / Task 3.10); equivalently $M_{\min} \sim \kappa \beta R c^2 / (2G)$ at fixed $\Delta = R$. Fuchs-regime interpretation (dense matter, low compactness) deferred as a refinement. |
...
- **Approach A (SXS rescaling, Varma et al. 2022).** $v_{\rm kick}^{\rm Fuchs} \sim v_{\rm kick}^{\rm BBH} \cdot \beta^2 \cdot C^{3/2}$, where $v_{\rm kick}^{\rm BBH}$ is the 5000 km/s record for near-equal-mass BBH merger. For Fuchs parameters ($\beta = 0.02$, $C = 0.44$) this gives $v_{\rm kick}^{\rm Fuchs} \sim 600$ m/s, or $\sim 10^{-4}$ of $v_{\rm warp}$. *(Session-37 NR verification: the 5000 km/s anchor upper-bounds every public SXS simulation — catalog-wide max remnant kick 3119.1 km/s, SXS:BBH:0662 — so this ceiling is conservative by ≥1.60×; `verification/test_sxs_kick_pull.py`.)*
...
A sweep (cell 13, `hf_jobs/sweeps/gw_recoil.py`) over $(\beta, C, M, n_{\rm orbits})$ returns a max of $10^{5.82}$ m/s $\approx 660$ km/s, at $\beta = 0.9$ and $C = 0.3$ — still $\sim 0.25\%$ of the warp target at those parameters. *(Session-37 corrections: the original phrasing said "$C = 0.5$" — the recorded artifact's argmax is $C = 0.3$, and the swept grids never included the $(0.9, 0.5)$ combination. The first-ever `gw_recoil_full.json` dispatch (4320 points, $\beta \le 0.99$, $C \le 0.9$; bit-exact regression on all 1200 recorded rows) tops out at $\Delta v / (\beta c) = 0.58\%$ within physical compactness $C \le 0.5$, and 1.41\% only at the unphysical $(\beta{=}0.99, C{=}0.9)$ corner; the $M$ axis is analytically degenerate in both formulas. Same order, same conclusion.)*
...
| DEC scaling law | $\Delta_{\min}/R = \kappa\,\beta/C$, analytic $\kappa \in [0.05, 0.875]$ (2A.9a) | $\rho_p^{\min} = -0.122\,\eta/\epsilon^2$ for ALL $\eta > 0$ |
