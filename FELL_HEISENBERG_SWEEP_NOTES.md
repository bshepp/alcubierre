...
## §19 Session 44 (2026-07-06) — the Π-exponent axis (2E.4's first axis) closed NEGATIVE

The FH radial-power exponent Π = 1/4 — the non-smooth point long suspected as the knob behind the §9 "all wall, no interior" pathology — was never varied in the whole Phase-2D arc. Swept Π ∈ {0.125, 0.2, 0.25, 0.3, 0.375, 0.5, 0.75, 1.0} at three certified anchor structures (A1, B1, S12 of §18) with the dual-box protocol (L=12 box ECs + passenger zone; L=45 far-field gate per the Session-42 rule), Npts=65. Baseline Π=0.25 regresses exactly (A1: wec +0.037405 / dec +0.018705 / L=45 −0.848).

| Finding | Result |
|---|---|
| Passenger zone vs Π | **Π-independent**: radius = h, volume = h³ (single voxel) at every Π ∈ [0.125, 1]; central \|N\| stays 13–22. The Π = 1/4 non-smoothness is NOT the driver of "all wall, no interior". |
| Box ECs (L=12) vs Π | Strict-pass survives only on the shelf Π ≲ 0.3 (DEC fails first at Π = 0.3: −0.19; both collapse for Π ≥ 0.375, to −52 at Π = 1). Mild monotone *improvement* toward small Π (wec +0.0396 at Π = 0.125 vs +0.0374 at 1/4) — box-scoped only. |
| Far-field gate (L=45) | **Negative at every Π** (−0.43 … −48), both asymmetry amplitudes (a = 0.2236 and 0.05). Structural reason: the far-field R-linear growth of φ comes from the (m+n)·R·erf₀ term, which carries no Π — the §18 global-EC failure extends across the whole Π axis. |

**Disposition: the anisotropic-Π exponent axis of 2E.4 is closed NEGATIVE** (within slice: adopted m,n concretization, the three anchor structures, Π ∈ [0.125, 1]). The remaining 2E.4 sub-axes (non-trivial topology; joint vortical+Π with independently-varied A-structure exponents) stay open with their recorded reopening criteria.

## §20 Session 50 (2026-07-12) — 2E.4 fully closed: topology (analytic) + joint vortical+Π (180-cell dual-box), both NEGATIVE

Battery [`verification/test_2e4_residual_axes.py`](verification/test_2e4_residual_axes.py) (topology 4/4; joint 5/5); sweep module [`hf_jobs/sweeps/fell_heisenberg_joint_2e4.py`](hf_jobs/sweeps/fell_heisenberg_joint_2e4.py); map `sweeps/fell_heisenberg_joint_2e4_full_concat.parquet`.

### §20.1 The far-field coefficients made exact (Π symbolic)

At $R \to \infty$ the adopted potential saturates exactly (GATE T1, symbolic; GATE T4 certifies the saturation against the exact φ at Π-scaled radii to 6.3e-15 — note the asymptotic regime starts at $R \sim ((m_0+a)(r+8\sqrt\sigma))^{1/2\Pi}$, i.e. ~10⁸ at Π = 0.125):

$$\varphi_{\rm sat} = -V\sqrt{\sigma\pi}\,R \;-\; V\sqrt{\sigma\pi}\,\frac{a}{m_0}\tanh(Z/\ell)\,R^{2\Pi}$$

- **R-linear coefficient $= -V\sqrt{\sigma\pi}$** — independent of Π, a, m₀, ℓ, r, and Z (GATE T2). This is the term behind the §17-era flat decay slope (non-decaying $|\vec N| \to V\sqrt{\sigma\pi}$) and it is removable by **no** parameter choice within the family except V = 0.
- **Growing anisotropic coefficient $\propto a\tanh(Z/\ell)$** — vanishes iff a = 0 (GATE T3), the fore-aft-symmetric case already marginal-inconclusive at the 1e-5 level (§18.3).

### §20.2 Topology sub-axis — closed NEGATIVE (analytic corollaries)

1. **Compact quotients (T³, lens spaces) admit no member of the adopted family.** The R-linear term makes φ incompatible with any closed 3-manifold: for $V \neq 0$ there is no global (periodic or closed-manifold) potential — the compact-space analogue of "no net charge". The only residual irrotational freedom on T³ is a harmonic 1-form = a **constant** shift (a global boost, no bubble); lens spaces have no harmonic 1-forms at all. The family is empty on compact topology except the trivial V = 0.
2. **Interior-only surgeries (handlebody / lens-space interior glued inside the bubble) cannot rescue global EC.** They leave the exterior untouched, and the §18 violation sits at finite equatorial $R^*$ *outside* any interior region — the global-EC verdict is topology-of-the-interior-independent.

Slice scope: the adopted family as defined by its flat-space potential formula (m,n concretization, a > 0). A genuinely new compact-topology warp ansatz *outside* this family is not asserted — that remains the reopening criterion.

### §20.3 Joint vortical+Π sub-axis — closed NEGATIVE (180-cell dual-box sweep)

Design: 3 certified anchors (A1, B1, S12) × background Π ∈ {0.125, 0.25, 0.5, 1} × {baseline + 3 amplitude patterns × 4 independently-varied A-exponents Π_A + 2 per-component exponent-split cells}, dual-box (L = 12 ECs + passenger zone; L = 45 far-field gate; A-fields normalised on the L = 12 grid and **reused** at L = 45 — one physical field, two windows). A1 Π = 0.25 baseline regresses exactly against the §19 row (wec +0.037405 / dec +0.018705 / far −0.848).

| Gate | Result |
|---|---|
| Far-field (L = 45) | **Negative at all 180/180 cells** — max far slack −0.431; per-Π extrema [−0.87, −0.43] (Π = 0.125) through [−48.1, −22.8] (Π = 1). No (Π, Π_A, amplitude, split) combination cancels the isotropic R-linear term (§20.1: it carries neither Π nor A). |
| Passenger zone | Single voxel (radius = h) at **every** cell — independently-varied A-exponents do not open it (extends §19's Π-independence and S38's vorticity result to the joint space). |
| Box slacks | **0/168 augmented cells** improve either slack over their (anchor, Π) baseline. |

**Disposition: Task 2E.4 is CLOSED in full** — Π axis (§19, S44), topology (§20.2, analytic), joint vortical+Π (§20.3, 180-cell dual-box). Slice: adopted m,n concretization, three certified anchors, Π, Π_A ∈ [0.125, 1], FH-form gradient-normalised A components, FD pipeline at Npts = 65 (validated for smooth FH fields, §18.1). Reopening criteria as recorded in ROADMAP 2E.4.
