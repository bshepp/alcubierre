...
**Last updated:** 2026-04-20 (Session 13 — Npts=97 resolution-convergence test refines the §7.7 cubic claim: the boundary is approximately degree-4-5 polynomial, not exactly cubic; Npts=65 sweep was systematically biased at low sigma; see [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §7.8).
...
|---|---|---|---|---|
| 1 | Shift profile is single-mode axisymmetric | **Load-bearing AND broken (Sessions 11-12 result).** | Slice 1: [`shift_families.ipynb`](shift_families.ipynb), [`SHIFT_FAMILIES_NOTES.md`](SHIFT_FAMILIES_NOTES.md). Multi-mode follow-up: [`fell_heisenberg.ipynb`](fell_heisenberg.ipynb), [`FELL_HEISENBERG2021_EVALUATION.md`](FELL_HEISENBERG2021_EVALUATION.md). Sweep + topology: [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md). | Single-mode axisymmetric: 0/140 sweep points achieve full WEC. **Multi-mode irrotational (Fell-Heisenberg 2021)**: Session 11 sweep found **1404 / 15000 grid points** strictly satisfying WEC+DEC ($|\vec{N}|_{\max}$ from $0.73c$ to $18.6c$, $E_{\rm neg} = 0$, resolution-converged). Session 12 connectivity analysis: the strict-pass set is **a single connected smooth-boundaried 5-D manifold** in $(\sigma, m_0, a, \ell, r)$, slack vanishes smoothly toward the boundary. The kinematic energy-condition bottleneck is solved within this static slice; remaining barriers are dynamical. |
...
| [`fell_heisenberg_topology_hires/`](fell_heisenberg_topology_hires/) | Session 13 outputs from Npts=97 resolution-convergence sweep. Same plot set as Session 12; better-resolved. |
...
## Open leads (ranked by signal-per-effort, post-Session-13)

**Note:** Session 11 completed the WEC+DEC sweep ([`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §1-§6); Session 12 added the connectivity/topology analysis (§7); Session 13 ran a Npts=97 resolution-convergence test (§7.8). The strict-pass set is **a single connected smooth-boundaried 5-D manifold** in $(\sigma, m_0, a, \ell, r)$ whose boundary is approximately a **degree-4-5 polynomial implicit surface** (not degree 3 as §7.7 originally claimed; Session 13 found the cubic claim was partially artifactual due to Npts=65 systematic bias at low sigma). The new top leads test whether the static-slice positive existence assembles into a complete physical drive.

1. **(Session-11-defined, still top priority — cheap and high signal.) Pointwise lapse-shift ratio $|\vec{N}|/\alpha$ as cheap horizon test (Task 2D.6).** Before the full horizon analysis, compute $|\vec{N}|/\alpha$ on the 3D grid for a representative WEC+DEC-passing winner. With unit lapse the threshold for the $t = \text{const}$ foliation to remain spacelike is $|\vec{N}| < 1$ pointwise. **If everywhere below 1, no horizon — done; the foliation is healthy and Task 2D.7 reduces to a CTC test only.** **If $\ge 1$ somewhere, that's exactly the locus to investigate.** A single line of NumPy at $\sim$0.1 sec per winner. Effort: <0.1 session.

2. **(REFINED, Session 13.) Extract the polynomial implicit boundary equation $P_d(\sigma, m_0, a, \ell, r) = 0$ at degree 4-5 (Task 2D.5b).** Session 13 refined the §7.7 finding: at Npts=97 the boundary classifier reaches 99.4% binary accuracy at degree 5 (vs 98.6% at degree 3), so the true boundary is **degree 4 or 5, not degree 3**. Refit logistic at degree 4 or 5 against the [`hf_jobs/configs/fell_heisenberg_refine_hires.json`](hf_jobs/configs/fell_heisenberg_refine_hires.json) Npts=97 parquet, extract the 125 or 251 coefficients, simplify symbolically, check for clean physical interpretation. Effort: 1 session, no new compute.

3. **(NEW, Session 13.) Convergence test at Npts=129 on a representative subset (Task 2D.5d).** Npts=97 is *better* than Npts=65 but not necessarily resolution-converged (canonical winner shows ~4% drift Npts=97→113). A targeted ~300-point Npts=129 sweep using the new `--points` mode would close the convergence question definitively. Effort: ~30 min cpu-xl, ~$0.20.

4. **(Session-11-defined.) Fell-Heisenberg full horizon and CTC analysis (Task 2D.7).** *Gated by Task 2D.6.* For the canonical WEC+DEC-passing point, compute (i) the norm of $\partial_t$ pointwise (foliation health, sharpened by 2D.6), (ii) outward null geodesic expansion at locations where 2D.6 flagged $|\vec{N}|/\alpha \to 1$, (iii) closed-timelike-curve test via Everett-Roman 1997 §4. The most likely place a "too good to be true" objection lands. Effort: 1-2 sessions if 2D.6 finds problem regions; 0.5 session if not.
...
7. **(Session-11-defined.) FH asymptotic-matching + double-bubble CTC test (Task 2D.10).** Box-edge matching to Schwarzschild/Minkowski exterior — does it require negative-energy Israel junction? Do two FH bubbles in opposite directions form CTCs? Combined effort: 1-2 sessions.
...
13. **(NEW, Session 13, deferred.) Hard Fix: symbolic extraction of the closed-form boundary equation (Task 2D.5e).** Documented in [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §8. Symbolic Hessian + ADM stress-energy + interior $(X,Y,Z)$ minimisation gives the exact transcendental boundary equation in closed form. Effort: **3-5 sessions** of heavy SymPy. Deferred unless polynomial fit (Task 2D.5b) yields unphysical-looking coefficients or other higher-priority leads exhaust.
