...
1. **(NEW, Session 11.) Fell-Heisenberg horizon and CTC analysis.** For the canonical WEC+DEC-passing point $(V=1.5, \sigma=10, m_0=3, a=0.05, \ell=4, r=9)$, compute (i) whether $\partial_t$ stays timelike everywhere in the box (foliation health), (ii) the expansion scalar of outward null geodesics (presence of trapped surfaces), (iii) closed-timelike-curve test via Everett-Roman 1997 §4. The most likely place a "too good to be true" objection lands. Effort: 1-2 sessions, mostly analytic, possibly some symbolic SymPy + targeted numerical Newton's-method root-finding. See [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §5 task 2.

2. **(NEW, Session 11.) Independent re-implementation of the FH ADM stress-energy pipeline.** Replicate the result in a second pipeline (e.g. SymPy + LAPACK in a different stencil, or Mathematica xAct verification) to rule out a systematic finite-difference bug in our specific stencil-of-stencils Hessian-of-Hessian computation. **The highest-value cheap check before any external claim is made.** Effort: 1-2 sessions. See [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §5 task 7.

3. **(NEW, Session 11.) Identify the FH source matter sector.** Given the WEC+DEC-passing $T_{\mu\nu}$, compute the principal-pressure profile $(\rho, p_1, p_2, p_3)$ as a field on the box and ask: what classical matter (perfect fluid, anisotropic fluid, plasma, scalar-field condensate) realises this stress profile? Place in the Bobrick-Martire 2021 §III taxonomy. The Fuchs et al. 2024 construction had this answered (explicit fluid shell); Fell-Heisenberg does not. Effort: 1 session. See [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §5 task 3.

4. **(NEW, Session 11.) FH asymptotic-matching + double-bubble CTC test.** Box-edge matching to Schwarzschild/Minkowski exterior — does it require negative-energy Israel junction? And do two FH bubbles in opposite directions form CTCs the way Krasnikov tubes do? Combined effort: 1-2 sessions. See [`FELL_HEISENBERG_SWEEP_NOTES.md`](FELL_HEISENBERG_SWEEP_NOTES.md) §5 tasks 4-5.

5. **Garattini-Zatrimaylov 2025 reproduction.** They claim a bubble at Hubble velocity in de Sitter satisfies averaged WEC/NEC. Our Slice 5 McVittie pipeline is already set up; adding a bubble on top and computing the averaged conditions is a focused extension. **Effort: 1-2 sessions.**

6. **Path 2B (Casimir / boundary-mode QFT) — start the programme.** Less urgent given Session 11 result (the energy-condition bottleneck doesn't require a quantum supplement after all, in the static slice), but Path 2B remains the right route for the *acceleration* and *dynamic-buildability* questions which Session 11's static result does not address. Tasks 2B.1–2B.5 in [`ROADMAP.md`](ROADMAP.md) are the natural starting points. **Effort: large (multiple sessions); largest scope.**

7. **Slice 4b (Krasnikov 2003 hybrid quantum/classical wall).** Reproduce Krasnikov's $10^{-3}$ g dihedral-portal + Van Den Broeck pocket and ask whether adding a Fuchs-class classical shell can eliminate the residual mg of QI-bounded negative energy. Focused, mostly computational. **Effort: 1 session.**

8. **TRUST_AUDIT #3 (Warp Factory + Fuchs Fig. 10 reproduction).** Last deferred audit item. Requires MATLAB on Windows. **Effort: 1 session of MATLAB wrestling.**

9. **Slice 6b (computational $f(R)$).** Build a 4th-order PDE solver. Significant new infrastructure. **Effort: large.**
