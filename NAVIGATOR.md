...
**Current state (2026-07-02, post-Session 31):** Phase 3.3+ — Fuchs §6 radial-profile optimization — is fully closed NEGATIVE in both its isotropic (Sessions 28–30) and anisotropic (Session 31) steps, and the exact-symbolic radial evaluator ([`warp_factory_py/solvers/axisymmetric_ec.py`](warp_factory_py/solvers/axisymmetric_ec.py)) is the certified EC oracle for sharp shell profiles (Session 30), with the Cartesian FD pipeline demoted to smooth-only qualitative cross-check. The one radial-certified positive finding of that arc — the Fuchs canonical mass is over-provisioned (plain constant-density passes all four ECs down to ADM ≈ 2.79e27 kg vs the canonical 4.49e27 kg) — has been promoted to the active lead (Task 3.10, certified minimal-mass mapping). The ranked open leads, the closed-leads ledger with reopening criteria, and outstanding admin live in [`ROADMAP.md`](ROADMAP.md); the session-by-session chronology is in [`SESSION_LOG.md`](SESSION_LOG.md); derivation-trust grades are in [`TRUST_AUDIT.md`](TRUST_AUDIT.md).
