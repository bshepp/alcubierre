...
2. **(NEW, Session 12.) Higher-order analytic sub-families inside $\mathcal{M}$ (Task 2D.5b, optional).** Session 12's symmetry probe ruled out clean low-order rational invariants but the strict-pass region's boundary is smooth and connected, so a higher-order analytic surface may still exist. Fit a smooth surface (polynomial regression or Gaussian process) to the 4686 boundary cells in [`fell_heisenberg_topology/boundary_cells.csv`](fell_heisenberg_topology/boundary_cells.csv) and ask whether the implicit equation simplifies. Effort: 1 session, no new compute.
