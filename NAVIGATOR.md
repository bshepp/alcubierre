...
1. **Phase 3.3 — nested + non-spherical Fuchs shells.** Stated as the next sub-task in [`ROADMAP.md`](ROADMAP.md) Phase 3 after Sessions 18-19 closed 3.1 and 3.2. **Misattribution clarification:** Fuchs et al. 2024 §5.2 ("Positive Energy Density") does *not* prescribe nested concentric shells — it only states the no-horizon constraint $R_{\rm shell} > 2GM_{\rm shell}/c^2$. The actual mass-reduction proposal is in §6 Conclusion: *"the smoothing process can be replaced by direct 1D optimization of the radial profiles for density, pressure, and shift vector, possibly reducing required mass by orders of magnitude"* — i.e. **single-shell radial-profile optimization, not nesting**. Sub-items 1–3 (NEC/WEC bug, TOV port, Fuchs Fig.10 reproduction) closed Session 25. **Sub-item 4 (nested shells) closed Session 26 NEGATIVE within slice**: at fixed $M_{tot}$ and fixed outer-wall warp band, splitting mass into an inner shell strictly degrades the NEC margin monotonically. Sub-item 5 (non-spherical shapes / oblate axisymmetric) is the next active task; sub-item 6 is final bookkeeping. Effort remaining: ~1 session for non-spherical test + bookkeeping.
