...
- **A "Slice 4b" hybrid quantum/classical computation is now open**, in principle. The setup would be: take our Task 2A.13 classical wall, suppose a small negative-energy quantum component is permitted (say, mg-scale per Krasnikov 2003 §3.3), and ask whether the combination is DEC-compatible. Expected outcome: the *spatial distribution* of the negative energy still matters — it has to coincide locally with where our classical analysis needs it. This is a genuinely interesting calculation that I'm deferring to a future session.
  ...
  **UPDATE (Session 23, 2026-05-12) — Slice 4b executed and closed NEGATIVE.** See [`krasnikov_hybrid.ipynb`](krasnikov_hybrid.ipynb) and [`KRASNIKOV_HYBRID_NOTES.md`](KRASNIKOV_HYBRID_NOTES.md). Within the Krasnikov-tube slice ($\eta \in [10^{-2}, 1)$, $\epsilon \in [10^{-2}, 1]\,\mathrm{m}$, $n=\rho_{\max}/\epsilon \in [2,100]$, tube length $D \ge 1\,\mathrm{m}$) the required negative-energy supplement to repair the pointwise DEC deficit of the classical wall **exceeds the §3.3 milligram budget by no fewer than 31 orders of magnitude** ($r_{\min} = 1.10 \times 10^{31}$ at $D=1\,\mathrm{m}$). The "spatial distribution" caveat above is the dominant effect: the deficit is $\mathcal{O}(1)$ in geometrized length$^{-2}$ over the entire wall band ($\sim$ a few $\epsilon$ wide at radius $\rho_{\max}$), and the geometrized-to-grams conversion factor $c^2/G \sim 10^{30}\,\mathrm{g/m}$ then dominates. Verification gates (i) Everett-Roman saturation match, (ii) universal $\epsilon^2$-collapse, (iii) Everett-Roman $\alpha$-band all pass. **The mg-scale §3.3 tail cannot serve as a quantum patch for the classical Krasnikov-tube wall in this slice.** Loopholes §3.1 and §3.2 (Weyl/Ricci ratio, sub-Planckian support) are *not* directly tested by Slice 4b and remain open as separate questions.
